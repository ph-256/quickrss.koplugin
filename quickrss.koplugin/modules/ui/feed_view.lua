...
function QuickRSSUI:_openFilterDialog()
    ...
    local menu_items = {
        -- Unread toggle
        {
            text = (Icons.BOOK or "") .. "  " .. (self.filter_unread
                and _("Showing Unread Only")
                or  _("Show Unread Only")),
            callback = function()
                self.filter_unread = not self.filter_unread
                self:_rebuildFooter() -- Fixed: Changed from _updateFilterButton
                self:_applyFilter()
                UIManager:close(dialog)
            end
        },
        -- Saved toggle
        {
            text = (Icons.SAVE or "") .. "  " .. (self.filter_saved
    ...
    for _, name in ipairs(feed_names) do
        local is_this_checked = (self.filter_feed == name)
        table.insert(menu_items, {
            text = (is_this_checked and check or "") .. name,
            checked = is_this_checked,
            callback = function()
